...
bool GeoNamesLMDB::openHandle(const std::string& path, LmdbHandle& h) {
    ...
    if (rc != MDB_SUCCESS) {
        LOG_F(ERROR, "[GeoNamesLMDB] mdb_env_create: %s", mdb_strerror(rc));
        return false;
    }
    rc = mdb_env_open(h.env, path.c_str(), MDB_RDONLY, 0664);
    if (rc != MDB_SUCCESS) {
        LOG_F(ERROR, "[GeoNamesLMDB] open(%s): %s", path.c_str(), mdb_strerror(rc));
...
bool GeoNamesLMDB::initialize(const std::string& lmdb_dir) {
    ...
    LOG_F(INFO, "[GeoNamesLMDB] Opening 7 LMDB databases from %s", dir.c_str());
    ...
    ready_.store(true);
    LOG_F(INFO, "[GeoNamesLMDB] Ready — admin1 + admin2 + postal + postal_reverse + cities + countries + aliases");
