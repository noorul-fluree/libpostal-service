...
void ParseController::parse(const drogon::HttpRequestPtr& req,
    ...
    if (!json || !json->isMember("address")) {
        ...
        LOG_F(WARNING, "[parse] Bad request: missing 'address' field");
    ...
    if (result.error.empty()) {
        LOG_F(INFO, "[parse] addr=\"%.80s\" confidence=%.2f latency=%.2fms cache=%s",
              raw_address.c_str(), result.confidence, result.latency_ms,
              result.from_cache ? "hit" : "miss");
    } else {
        LOG_F(WARNING, "[parse] FAILED addr=\"%.80s\" error=\"%s\" latency=%.2fms",
              raw_address.c_str(), result.error.c_str(), result.latency_ms);
    }
...
void ParseController::normalize(const drogon::HttpRequestPtr& req,
    ...
    LOG_F(INFO, "[normalize] addr=\"%.80s\" expansions=%zu latency=%.2fms",
             raw.c_str(), result.normalizations.size(), result.latency_ms);
