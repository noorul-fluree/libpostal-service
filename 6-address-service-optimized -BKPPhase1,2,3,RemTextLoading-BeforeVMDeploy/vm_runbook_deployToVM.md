...
| `drogon/examples/*/config.json` | Drogon framework examples | **NO — irrelevant** |
...
│   ├── config/config.json        ← LIVE config (only edit this)
│   ├── data/libpostal/           ← standard model data
│   ├── data/libpostal_senzing/   ← senzing model data
│   ├── logs/service.log          ← live logs
│   └── run.sh                    ← start script
├── address-service-build/        ← SOURCE + BUILD
│   ├── src/                      ← drop .cc/.h files here via WinSCP
│   ├── config/config.json        ← template only, never edit
│   └── build/                    ← cmake output, run make here
├── drogon/                       ← Drogon framework source (compiled once)
└── libpostal/                    ← libpostal source (compiled once or twice)
```
...
### .cc or .h files only (drop via WinSCP into address-service-build/src/)
```bash
cd /home/flureelabs/address-parser-service/address-service-build/build
make -j$(nproc)
cp address-service /home/flureelabs/address-parser-service/address-service/bin/
pkill -f "address-service"
cd /home/flureelabs/address-parser-service/address-service
nohup bash run.sh > logs/service.log 2>&1 &
```

### Both .cc/.h AND config.json changed
```bash
# Same as .cc/.h change — make picks up code, config picked up on restart
cd /home/flureelabs/address-parser-service/address-service-build/build
make -j$(nproc)
cp address-service /home/flureelabs/address-parser-service/address-service/bin/
pkill -f "address-service"
...
### CMakeLists.txt changed (new file added, new library, new option)
```bash
cd /home/flureelabs/address-parser-service/address-service-build/build
cmake3 .. \
...
## Batch/Enrich — Over-limit Behaviour

Both `/api/v1/batch` and `/api/v1/enrich` **no longer reject** requests that exceed `batch_max_size`. Instead they:

1. Process only the first `batch_max_size` records
2. Silently discard the rest
3. Return a `warning` field in the response with full details

Example response when 6000 records sent but max is 5000:
```json
{
  "total": 5000,
  "succeeded": 5000,
...
To change the max — edit live config only, then restart:
```bash
nano /home/flureelabs/address-parser-service/address-service/config/config.json
# change: "max_size": 5000  to whatever you need
pkill -f "address-service"
cd /home/flureelabs/address-parser-service/address-service
nohup bash run.sh > logs/service.log 2>&1 &
```
