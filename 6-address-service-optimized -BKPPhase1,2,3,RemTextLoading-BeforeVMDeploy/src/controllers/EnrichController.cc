...
void EnrichController::enrich(const drogon::HttpRequestPtr& req,
    ...
    LOG_F(INFO, "[enrich] ▶ records_in=%d processing=%d max_allowed=%d",
                                 N, process_N, g_config.batch_max_size);
    ...
    LOG_F(INFO, "[enrich] records=%d succeeded=%d failed=%d latency=%.2fms",
        //   N, succeeded, failed, latency);
           process_N, succeeded, failed, latency);
    if (failed > 0)
        LOG_F(WARNING, "[enrich] %d record(s) failed validation", failed);
