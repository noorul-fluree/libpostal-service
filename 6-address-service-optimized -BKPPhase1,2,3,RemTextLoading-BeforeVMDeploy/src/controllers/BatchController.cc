...
void BatchController::batchParse(const drogon::HttpRequestPtr& req,
    ...
    // Do not reject — if over limit, slice to max and warn in response

    BatchRequest batch_req = BatchRequest::fromJson(*json);
    const auto& all_addresses = batch_req.addresses;
    const size_t total_received = all_addresses.size();

    // Slice to max if over limit
    bool truncated = false;
    size_t N = total_received;
    if ((int)total_received > g_config.batch_max_size) {
        N = static_cast<size_t>(g_config.batch_max_size);
        truncated = true;
    }
    // Use only first N addresses
    std::vector<std::string> addresses(all_addresses.begin(),
                                       all_addresses.begin() + N);

    std::cout << "[BatchController] ▶ POST /api/v1/batch | records_in=" << total_received
              << " | processing=" << N
              << " | max_allowed=" << g_config.batch_max_size << "\n";
    LOG_F(INFO, "[batch] ▶ records_in=%zu processing=%zu max_allowed=%d",
      total_received, N, g_config.batch_max_size);        
    ...
    for (size_t i = 0; i < N; ++i) {
       ...
       if (!batch_resp.results[i].from_cache) {
    ...
    if (batch_resp.failed > 0)
        ...
        LOG_F(INFO, "[batch] records=%d succeeded=%d failed=%d unique_parsed=%zu latency=%.2fms",
          (int)N, batch_resp.succeeded, batch_resp.failed,
          to_parse.size(), batch_resp.total_latency_ms);
    if (batch_resp.failed > 0)
        LOG_F(WARNING, "[batch] %d record(s) failed validation", batch_resp.failed);

    metrics.recordBatch(batch_resp.total, batch_resp.total_latency_ms);

    Json::Value resp_json = batch_resp.toJson();
    if (truncated) {
        resp_json["warning"] = "Received " + std::to_string(total_received) +
                               " records but maximum is " +
                               std::to_string(g_config.batch_max_size) +
                               ". Only first " + std::to_string(N) +
                               " records processed. Remaining " +
                               std::to_string(total_received - N) +
                               " records were discarded.";
        resp_json["total_received"]  = static_cast<int>(total_received);
        resp_json["total_processed"] = static_cast<int>(N);
        resp_json["total_discarded"] = static_cast<int>(total_received - N);
    }
