...
bool AddressParser::initialize(const std::string& data_dir) {
    ...
    // std::call_once(init_flag_, [&]() { BEFORE SENZING MODEL:
    //     if (!data_dir.empty())
...
bool AddressParser::initLanguageClassifier() {
    ...
    // if (!libpostal_setup_language_classifier()) {  // Before SENZING MODEL: - other wise uncommen this and remove below two lines.
    const char* dir = !data_dir_.empty() ? data_dir_.c_str() : nullptr;
