...
//
//  normalize_cols empty  → include ALL parsed fields
//  normalize_cols non-empty → include only those parsed fields
//
...
{
    Json::Value out;
    const bool all = normalize_cols.empty(); // true = ALL mode
    ...
    // normalize_cols only controls which *parsed* fields get appended below.
    out = raw;
...
void EnrichController::enrich(const drogon::HttpRequestPtr& req,
    ...
    if (!json->isMember("records") || !(*json)["records"].isArray()) {
        Json::Value err;
        err["error"] = "Missing or invalid 'records' array";
        auto r = drogon::HttpResponse::newHttpJsonResponse(err);
        r->setStatusCode(drogon::k400BadRequest);
        callback(r);
        return;
    ...
    const Json::Value& records = (*json)["records"];
    ...
    // address_column
    std::string addr_col;
    if (json->isMember("address_column") && (*json)["address_column"].isString())
        addr_col = (*json)["address_column"].asString();
    ...
    // key_column (informational — kept as passthrough, not required)
    std::string key_col;
    if (json->isMember("key_column") && (*json)["key_column"].isString())
        key_col = (*json)["key_column"].asString();
    if (key_col.empty()) key_col = detectKeyColumn(first, addr_col);

    // normalize_columns
    std::string norm_spec = "ALL";
    if (json->isMember("normalize_columns") && (*json)["normalize_columns"].isString())
        norm_spec = (*json)["normalize_columns"].asString();
    auto norm_cols = parseNormalizeColumns(norm_spec);

    // language / country hints
    std::string lang = json->isMember("language") ? (*json)["language"].asString()
                                                   : g_config.default_language;
    std::string ctry = json->isMember("country")  ? (*json)["country"].asString()
